    ...
    <div class="row">
      <div class="col-md-12">
        <section class="tile">
          <div class="pageheader tile-bg">
            ...
            <div class="page-bar">
              <div class="btn-toolbar pull-right">
                <div class="btn-group">
                  <?php
                    $brthDat=$this->db->query("select * from tbl_campaign where type='Anniversary' ");
                    $num_rows=$brthDat->num_rows();
                    if($num_rows > 0) { ?>
                  <button class="btn btn-danger btn-campaign" onclick="return confirm('Anniversary campaign already created . You can not create more than one campaign !');" style="margin: 17px 0px 0px 0px;">CREATE CAMPAIGN</button>
                  <?php } else { ?>
                  <button class="btn btn-danger btn-campaign" data-toggle="modal" data-target="" formid="#anniversaryForm" id="formresetId" style="margin: 17px 0px 0px 0px;">CREATE CAMPAIGN</button>
                  <?php } ?>
                </div>
          ...
          <h4 style="margin: 15px;">Anniversary Campaign</h4>
                        ...
                        <div class="panel-group icon-plus" id="accordion" role="tablist" aria-multiselectable="true">
                          <div class="panel panel-default panel-transparent">
                            <div class="panel-heading" role="tab" id="nameandoccupation33">
                              <h4 class="panel-title"> <a data-toggle="collapse" href="#nameandoccupation1" aria-expanded="true"> <span><i class="fa fa-minus text-sm mr-5"></i>Anniversary Wishes</span> </a> </h4>
                            </div>
                                        ...
                                        <div class="table-responsive">
                                          <table class="table mb-0">
                                            <tbody>
                                              <?php 
                                                $i=1;
                                                $wishes=$this->db->query("select * from tbl_template where type='Anniversary' AND sub_type='Anniversary Wishes' ");
                                  ...
                                  <div class="table-responsive">
                                    <table class="table mb-0" id="usersList">
                                      ...
                                      <tbody>
                                        <?php 
                                          $i=1;
                                          $disount=$this->db->query("select * from tbl_campaign where type='Anniversary' AND maker_id='".$this->session->userdata('user_id')."' ");
                                          foreach ($disount->result() as $getDiscount) {
                                          ?>
                                        <tr class="record">
                                          <td><?php echo $getDiscount->title; ?></td>
                                          <td style="width: 50%"><?php echo $getDiscount->wishes_text; ?></td>
                                          <td><?php echo $getDiscount->maker_date; ?></td>
                                          <td><?php echo $getDiscount->vaild_till; ?></td>
                                          <td>
                                            <?php $msgLog=$this->db->query("select * from tbl_message_log where type='$getDiscount->type' AND sub_type='$getDiscount->sub_type' "); 
                                              $count=$msgLog->num_rows(); ?>
                                            <a href="<?=base_url('Campaign/anniversary_sms_log');?>" target="_blank">
                                              <center><?=$count?></center>
                                            </a>
                                          </td>
                                          <td>
                                            ...
                                            <div class="btn-group pull-right">
                                              ...
                                              <ul class="dropdown-menu" role="menu">
                                                <li><a href="#" onclick="edit_anniversary_wishes(this);" property = "edit" type="button" data-toggle="modal" data-target="#anniversaryModal" arrt= '<?=json_encode($getDiscount);?>'  data-backdrop='static' data-keyboard='false'><i class="fa fa-edit"></i> Edit This Campaign</a></li>
...
<div class="modal fade" id="anniversaryModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
        <h4 class="modal-title"><span class="top_title">Create</span> Anniversary Campaign</h4>
        <div id="resultanniversary" style="font-size: 15px;color: red; text-align:center;"></div>
      </div>
...
<script>
  ...
  function check(ths)
   ...
   $('#formresetId').click(function(){
     ...
     {
       $("#formresetId").attr('data-target','#anniversaryModal'); 
     }
